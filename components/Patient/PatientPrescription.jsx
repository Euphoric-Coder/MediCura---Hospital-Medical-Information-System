...
import { Plus, Pill, Search, Download, Eye, Calendar, User, Clock, AlertTriangle, CheckCircle, X, RefreshCw } from 'lucide-react';
      ...
      <div className="bg-dark-400 border border-dark-500 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 lg:p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-20-bold lg:text-24-bold text-white">Prescription Details</h2>
            <button
              onClick={onClose}
          ...
          <div className="bg-gradient-to-r from-purple-500/10 to-purple-600/5 backdrop-blur-sm border border-purple-500/20 rounded-2xl p-6 mb-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <h3 className="text-18-bold lg:text-20-bold text-white">{prescription.medication}</h3>
                <p className="text-14-regular text-purple-400">{prescription.dosage} - {prescription.frequency}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-14-regular text-dark-700">
              ...
              <div>
                <span className="text-white">Start Date:</span> {prescription.startDate}
              </div>
              <div>
                <span className="text-white">End Date:</span> {prescription.endDate}
              </div>
            </div>
          </div>
          ...
          <div className="bg-dark-500/30 rounded-2xl p-6 mb-6">
            <h4 className="text-16-bold lg:text-18-bold text-white mb-4">Refill Information</h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-14-regular text-dark-700">
              <div>
                <span className="text-white">Refills Left:</span> {prescription.refillsLeft}
              </div>
              <div>
                <span className="text-white">Total Refills:</span> {prescription.totalRefills}
              </div>
              <div>
              ...
              {prescription.lastRefillDate && (
                <div>
                  <span className="text-white">Last Refill:</span> {prescription.lastRefillDate}
                </div>
              )}
              {prescription.nextRefillDate && (
                <div>
                  <span className="text-white">Next Refill:</span> {prescription.nextRefillDate}
                </div>
              )}
              <div>
                <span className="text-white">Pharmacy:</span> {prescription.pharmacy}
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-500/10 rounded-2xl p-6 mb-6">
            <h4 className="text-16-bold lg:text-18-bold text-white mb-4">Instructions</h4>
            <p className="text-14-regular text-blue-300">{prescription.instructions}</p>
          </div>

          {/* Side Effects */}
          {prescription.sideEffects.length > 0 && (
            <div className="bg-red-500/10 rounded-2xl p-6 mb-6">
              <h4 className="text-16-bold lg:text-18-bold text-white mb-4">Possible Side Effects</h4>
...
const PatientPrescriptions = ({ onBack }) => {
  const [prescriptions] = useState([
    ...
    },
    {
      ...
      refillsLeft: 1,
      totalRefills: 2,
      instructions: 'Take with meals to reduce stomach upset. Monitor blood sugar levels. Stay hydrated.',
    ...
    },
    {
      ...
      refillsLeft: 0,
      totalRefills: 1,
      instructions: 'Take with food. Do not exceed 3 doses per day. Discontinue if stomach upset occurs.',
  ...
  const generatePrescriptionPDF = (prescription) => {
    ...
    doc.text('For questions about this prescription, contact your healthcare provider.', 20, 290);
  ...
  const pendingRefills = prescriptions.filter(p => p.status === 'active' && p.refillsLeft > 0).length;
  const totalCost = prescriptions.filter(p => p.status === 'active').reduce((sum, p) => sum + p.cost, 0);
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
            ...
            <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 backdrop-blur-sm border border-yellow-500/20 rounded-2xl p-4 lg:p-6">
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                <div>
                  <div className="text-20-bold lg:text-32-bold text-white">{pendingRefills}</div>
                  <div className="text-10-regular lg:text-14-regular text-yellow-400">Refillable</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 backdrop-blur-sm border border-purple-500/20 rounded-2xl p-4 lg:p-6">
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                <div>
                  ...
                  <div className="text-10-regular lg:text-14-regular text-purple-400">Monthly Cost</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-8">
            <div className="flex items-center gap-3 mb-6 lg:mb-8">
              ...
              <h2 className="text-20-bold lg:text-24-bold text-white">Your Medications</h2>
            </div>
