...
import { Plus, BarChart3, TrendingUp, TrendingDown, Users, Bed, DollarSign, Calendar, Download, Filter } from 'lucide-react';
...
const AdminReportsAnalytics = ({ onBack }) => {
  ...
  const generateHospitalReport = () => {
    ...
    doc.text('Phone: (555) 123-4567 | Email: admin@carepulse.com', 20, 50);
    ...
    doc.save(`hospital-report-${selectedPeriod}-${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const BarChart = ({ data, title, unit = '' }) => {
    ...
    return (
      <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-6">
        <h3 className="text-16-bold lg:text-20-bold text-white mb-4 lg:mb-6">{title}</h3>
        <div className="space-y-4">
          {data.map((item, index) => (
            <div key={index} className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-12-medium lg:text-14-medium text-white">{item.label}</span>
                <div className="flex items-center gap-2">
                  <span className="text-14-semibold lg:text-16-semibold text-white">
                    {unit === '$' ? `$${(item.value / 1000000).toFixed(1)}M` : 
                     unit === '%' ? `${item.value}%` : 
                     `${item.value}${unit}`}
                  </span>
                  <div className={`flex items-center gap-1 px-2 py-1 rounded-full ${
                    ...
                    <span className="text-10-medium lg:text-12-medium">{Math.abs(item.change)}%</span>
                  </div>
                </div>
              </div>
              <div className="w-full bg-dark-500 rounded-full h-2 lg:h-3">
                <div 
                  ...
                  style={{ width: `${unit === '%' ? item.value : (item.value / maxValue) * 100}%` }}
                />
              </div>
            </div>
          ))}
  ...
  const StatCard = ({ title, value, change, icon: Icon, color }) => (
    <div className={`bg-gradient-to-br ${color} backdrop-blur-sm border border-opacity-20 rounded-2xl p-4 lg:p-6`}>
      <div className="flex items-center gap-3 lg:gap-4">
        <div className={`w-10 h-10 lg:w-14 lg:h-14 bg-gradient-to-r ${color.replace('/10', '').replace('/5', '')} rounded-2xl flex items-center justify-center shadow-lg`}>
          <Icon className="w-5 h-5 lg:w-7 lg:h-7 text-white" />
        </div>
        <div>
          <div className="text-20-bold lg:text-32-bold text-white">{value}</div>
          <div className="text-10-regular lg:text-14-regular text-opacity-70">{title}</div>
          <div className={`flex items-center gap-1 mt-1 ${
            ...
            <span className="text-10-medium lg:text-12-medium">{Math.abs(change)}% vs last {selectedPeriod}</span>
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="text-20-bold lg:text-24-bold text-white">Reports & Analytics</span>
                <p className="text-12-regular lg:text-14-regular text-dark-700">Hospital performance insights</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
        ...
        {selectedReport === 'overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <BarChart data={admissionsData} title="Monthly Patient Admissions" />
            <BarChart data={occupancyData} title="Department Occupancy Rates" unit="%" />
            <BarChart data={revenueData} title="Quarterly Revenue" unit="$" />
            <BarChart data={staffWorkloadData} title="Staff Workload (Hours/Week)" />
          </div>
        )}
        ...
        {selectedReport === 'admissions' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-6">
              <h3 className="text-16-bold lg:text-20-bold text-white mb-4 lg:mb-6">Admission Sources</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-12-medium lg:text-14-medium text-white">{item.source}</span>
                      <span className="text-14-semibold lg:text-16-semibold text-white">{item.count}</span>
                    </div>
        ...
        {selectedReport === 'occupancy' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <BarChart data={occupancyData} title="Current Occupancy by Department" unit="%" />
            <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-6">
              <h3 className="text-16-bold lg:text-20-bold text-white mb-4 lg:mb-6">Bed Utilization</h3>
              <div className="space-y-4">
                {[
                  { dept: 'ICU', total: 20, occupied: 17, available: 3 },
                  { dept: 'General Ward', total: 100, occupied: 72, available: 28 },
                  ...
                  <div key={index} className="bg-dark-500/30 rounded-lg p-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-14-medium text-white">{item.dept}</span>
                      <span className="text-12-regular text-dark-700">{item.occupied}/{item.total} beds</span>
                    </div>
                    <div className="w-full bg-dark-500 rounded-full h-2">
                      <div 
        ...
        {selectedReport === 'revenue' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            <BarChart data={revenueData} title="Quarterly Revenue Performance" unit="$" />
            <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-6">
              <h3 className="text-16-bold lg:text-20-bold text-white mb-4 lg:mb-6">Revenue Breakdown</h3>
              <div className="space-y-4">
                {[
                  { category: 'Inpatient Services', amount: 1450000, percentage: 50 },
                  { category: 'Outpatient Services', amount: 870000, percentage: 30 },
                  { category: 'Emergency Services', amount: 435000, percentage: 15 },
                  { category: 'Pharmacy', amount: 145000, percentage: 5 }
                ].map((item, index) => (
                  <div key={index} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-12-medium lg:text-14-medium text-white">{item.category}</span>
                      <span className="text-14-semibold lg:text-16-semibold text-white">${(item.amount / 1000).toFixed(0)}K</span>
                    </div>
                    <div className="w-full bg-dark-500 rounded-full h-2">
                      <div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 mt-6 lg:mt-8">
          <div className="bg-gradient-to-r from-blue-500/10 to-blue-600/5 backdrop-blur-xl border border-blue-500/20 rounded-3xl p-4 lg:p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-16-bold lg:text-18-bold text-white">Patient Satisfaction</h3>
            </div>
            <div className="text-24-bold lg:text-32-bold text-white mb-2">4.6/5.0</div>
          ...
          <div className="bg-gradient-to-r from-green-500/10 to-green-600/5 backdrop-blur-xl border border-green-500/20 rounded-3xl p-4 lg:p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-16-bold lg:text-18-bold text-white">Readmission Rate</h3>
            </div>
            <div className="text-24-bold lg:text-32-bold text-white mb-2">8.2%</div>
            <div className={`flex items-center gap-1 text-green-400`}>
          ...
          <div className="bg-gradient-to-r from-purple-500/10 to-purple-600/5 backdrop-blur-xl border border-purple-500/20 rounded-3xl p-4 lg:p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-16-bold lg:text-18-bold text-white">Staff Efficiency</h3>
            </div>
            <div className="text-24-bold lg:text-32-bold text-white mb-2">92%</div>
            <div className={`flex items-center gap-1 text-green-400`}>
              <TrendingUp className="w-4 h-4" />
