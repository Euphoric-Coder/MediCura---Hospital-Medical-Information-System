...
import { Plus, TestTube, Search, Calendar, User, Clock, AlertTriangle, CheckCircle, Eye, Download, Edit, X } from 'lucide-react';
import jsPDF from 'jspdf';

const availableTests = [
  { id: '1', name: 'Complete Blood Count (CBC)', category: 'Hematology', cost: 85 },
  { id: '2', name: 'Basic Metabolic Panel', category: 'Chemistry', cost: 65 },
  { id: '3', name: 'Lipid Profile', category: 'Chemistry', cost: 75 },
  { id: '4', name: 'Thyroid Function Tests', category: 'Endocrinology', cost: 120 },
      ...
      <div className="bg-dark-400 border border-dark-500 rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        <div className="p-4 sm:p-6 lg:p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-18-bold sm:text-20-bold lg:text-24-bold text-white">Order Lab Tests</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Test Selection */}
            <div>
              <label className="shad-input-label block mb-2">Select Tests</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-64 overflow-y-auto">
                {availableTests.map((test) => (
                  <label key={test.id} className="flex items-center gap-3 cursor-pointer p-3 rounded-lg hover:bg-green-500/10 transition-colors border border-dark-500">
                    <input
                      type="checkbox"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-12-regular text-green-400">{test.category} - ${test.cost}</div>
                    </div>
                  </label>
                ))}
              </div>
              ...
              <div className="flex gap-4">
                {['routine', 'urgent', 'stat'].map((priority) => (
                  <label key={priority} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-14-regular text-white capitalize">{priority}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {/* Reason */}
            <div>
              <label className="shad-input-label block mb-2">Reason for Tests</label>
              <textarea
                value={orderData.reason}
                onChange={(e) => setOrderData(prev => ({ ...prev, reason: e.target.value }))}
...
const DoctorLabOrders = ({ onBack }) => {
  ...
  const generateLabOrderPDF = (order) => {
    ...
    doc.save(`lab-order-${order.testName.toLowerCase().replace(/\s+/g, '-')}-${order.id}.pdf`);
  };

  const handleNewOrder = (orderData) => {
    const selectedTests = availableTests.filter(test => orderData.tests.includes(test.id));
    ...
    selectedTests.forEach((test, index) => {
      const newOrder = {
        ...
        cost: test.cost,
        lab: test.category === 'Radiology' ? 'Radiology Department' : 'CarePulse Laboratory',
    ...
    setMessage(`${selectedTests.length} lab test(s) ordered for ${orderData.patientName}`);
  ...
  const getStatusBadge = (status, priority) => {
    ...
    switch (status) {
      case 'ordered':
        return (
          <div className={`${baseClasses} ${priority === 'urgent' ? 'bg-yellow-500/20 border border-yellow-500/30' : 'bg-blue-500/20 border border-blue-500/30'}`}>
            <Clock className="w-3 h-3" />
            <span className={priority === 'urgent' ? 'text-yellow-400' : 'text-blue-400'}>
      ...
      case 'collected':
        return (
          <div className={`${baseClasses} bg-purple-500/20 border border-purple-500/30`}>
  ...
  const urgentCount = labOrders.filter(o => o.priority === 'urgent' || o.priority === 'stat').length;
  const totalCost = labOrders.filter(o => o.status === 'completed').reduce((sum, o) => sum + o.cost, 0);
          ...
          <div className="max-w-7xl mx-auto px-4 lg:px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-20-bold lg:text-24-bold text-white">Lab Orders</span>
                  <p className="text-12-regular lg:text-14-regular text-dark-700">Order and track lab tests</p>
                </div>
              </div>
              <button
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 lg:py-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
            <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 backdrop-blur-sm border border-blue-500/20 rounded-2xl p-4 lg:p-6">
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                <div>
                  <div className="text-20-bold lg:text-32-bold text-white">{orderedCount}</div>
                  <div className="text-10-regular lg:text-14-regular text-blue-400">Ordered</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 backdrop-blur-sm border border-purple-500/20 rounded-2xl p-4 lg:p-6">
              <div className="flex items-center gap-3 lg:gap-4">
                ...
                <div>
                  <div className="text-20-bold lg:text-32-bold text-white">${totalCost.toFixed(0)}</div>
          ...
          <div className="bg-gradient-to-r from-dark-400/30 to-dark-300/30 backdrop-blur-xl border border-dark-500/50 rounded-3xl p-4 lg:p-8">
            <div className="flex items-center gap-3 mb-6 lg:mb-8">
              ...
              <h2 className="text-18-bold lg:text-24-bold text-white">Lab Test Orders</h2>
            </div>

            <div className="space-y-4">
              {filteredOrders.map((order) => (
                <div key={order.id} className="bg-gradient-to-r from-dark-300/50 to-dark-400/30 backdrop-blur-sm border border-dark-500/50 rounded-2xl p-4 lg:p-6 hover:border-dark-500/80 transition-all duration-300">
                    ...
                    <div className="flex items-center gap-4 lg:gap-6">
                      ...
                      <div className="space-y-2 min-w-0 flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                          <h3 className="text-16-bold lg:text-20-bold text-white">{order.testName}</h3>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-12-regular lg:text-14-regular text-dark-700">
                          <div>
                            <span className="text-white">Patient:</span> {order.patientName}
                          </div>
                          <div>
                            <span className="text-white">Category:</span> {order.category}
                          </div>
                          <div className="hidden sm:block">
                            <span className="text-white">Cost:</span> ${order.cost}
                          </div>
                        </div>
                        
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-12-regular lg:text-14-regular text-dark-700">
                          <div>
                            <span className="text-white">Ordered:</span> {order.orderedDate}
                          </div>
                          {order.collectionDate && (
                            <div>
                              <span className="text-white">Collected:</span> {order.collectionDate}
                            </div>
                          )}
                          {order.resultDate && (
                            <div>
                              <span className="text-white">Result:</span> {order.resultDate}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="bg-dark-500/30 rounded-lg px-3 py-2">
                          <p className="text-10-regular lg:text-12-regular text-dark-600">
                            <span className="text-white">Reason:</span> {order.reason}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            {filteredOrders.length === 0 && (
              <div className="text-center py-12 lg:py-20">
                ...
                <h3 className="text-20-bold lg:text-24-bold text-white mb-4">No lab orders found</h3>
